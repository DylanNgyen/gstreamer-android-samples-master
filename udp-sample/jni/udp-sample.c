...
app_function (void *userdata)
{
    ...
    /* Build pipeline */
    data->pipeline =
            gst_parse_launch("udpsrc port=8555 caps=\"application/x-rtp, media=video, encoding-name=H264, clock-rate=90000\" ! rtph264depay ! decodebin ! videoconvert ! autovideosink", &error);
    if (error) {
        gchar *message =
...
gst_native_init (JNIEnv * env, jobject thiz)
{
    ...
    GST_DEBUG_CATEGORY_INIT (debug_category, "udp-sample", 0,
                             "UDP Sample");
    gst_debug_set_threshold_for_name ("udp-sample", GST_LEVEL_DEBUG);
...
gst_native_class_init (JNIEnv * env, jclass klass)
{
    ...
    if (!custom_data_field_id || !set_message_method_id
         ...
         */
        __android_log_print (ANDROID_LOG_ERROR, "udp-sample",
...
JNI_OnLoad (JavaVM * vm, void *reserved)
{
    ...
    jclass klass = (*env)->FindClass (env,
                                      "org/freedesktop/gstreamer/tutorials/udp_sample/UDPSample");
